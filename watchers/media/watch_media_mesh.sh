...
# ALMS MEDIA MESH WATCHER — V1
# PURPOSE: Expand URLs → extract root domain → generate intake JSON
...
# 2. Extract host/root surface. Registrable-domain reduction is handled by later clusterer.
ROOT_DOMAIN="$(printf '%s\n' "$RESOLVED_URL" | awk -F[/:] '{print $4}')"
