#!/usr/bin/env bash
set -euo pipefail
TARGET_URL="https://www.cloudflare.com/plans/"
SOURCE_ID="cloudflare_plans_page"
...
DIFF_SUMMARY=""
if [[ -n "${PREV_NORM_ARTIFACT}" ]] && [[ -f "${PREV_NORM_ARTIFACT}" ]]; then
  PREV_WORDS="$(wc -w < "${PREV_NORM_ARTIFACT}")"
  CURR_WORDS="$(wc -w < "${NORM_FILE}")"
  DIFF=$((CURR_WORDS - PREV_WORDS))
  if [[ $DIFF -gt 0 ]]; then
    DIFF_SUMMARY="+${DIFF} words added"
  elif [[ $DIFF -lt 0 ]]; then
    DIFF_SUMMARY="${DIFF} words removed"
  else
    DIFF_SUMMARY="content modified, word count unchanged"
  fi
fi
...
cat > "$CANDIDATE_FILE" <<JSON
{
  ...
  "evidence": {
    ...
    "normalized_hash": "sha256:${NORM_HASH}",
    "previous_normalized_hash": "${PREV_HASH}",
    "diff_summary": "${DIFF_SUMMARY}",
    "artifact_path": "${RAW_DIR}/${TIMESTAMP}.html"
  },
  "claim": {
    "subject": "${SOURCE_ID}",
...
jq --arg ts "$TIMESTAMP" \
   --arg hash "sha256:${NORM_HASH}" \
   --arg raw "${RAW_FILE}" \
   --arg norm "${NORM_FILE}" \
   --arg verified_hash "$HASH" \
   --arg candidate "${CANDIDATE_FILE}" \
  ".${SOURCE_ID}.last_detected_at = \$ts
   | .${SOURCE_ID}.last_normalized_hash = \$hash
   | .${SOURCE_ID}.last_raw_artifact_path = \$raw
   | .${SOURCE_ID}.last_normalized_artifact_path = \$norm
   | .${SOURCE_ID}.last_verified_hash = \$verified_hash
   | .${SOURCE_ID}.last_candidate_path = \$candidate" \
