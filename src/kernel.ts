 ...
 *
 * A small auditable machine for replay-authoritative records.
 *
 * Components:
...
interface ReplayResult {
  root: Hash;
  eventCount: number;
  checkpoints: { index: number; root: Hash }[];
  manifestUsed: Hash;
  degraded: boolean;
...
function enforceNumberPolicy(obj: unknown, policy: "forbidden" | "decimal-string"): unknown {
  if (typeof obj === "number") {
    if (!Number.isSafeInteger(obj) || policy === "forbidden") throw new Error(`Number forbidden by manifest: ${obj}`);
...
function merkleRoot(state: Map<string, string>): Hash {
...
class CanonicalizationEngine {
  constructor(public readonly manifest: CanonicalizationManifest) {
    this.validate();
  }
  ...
  canonicalize(raw: RawEvent, parentHash: Hash): CanonicalEvent {
    ...
    return {
      id: sha256Sync(canonicalPayload),
      index: raw.index,
      canonicalPayload,
      manifestUsed: this.manifest.hash,
      parentHash
    };
  }
}
...
class MinimalVerifiableKernel {
  ...
  replay(manifestHash?: Hash): ReplayResult {
    ...
    for (const event of this.log.events) {
      ...
      if (event.index % 1000 === 0) checkpoints.push({ index: event.index, root: merkleRoot(state) });
    }

    return {
      root: merkleRoot(state),
      eventCount: this.log.events.length,
      checkpoints,
      manifestUsed: targetManifestHash,
      degraded: degradationNotes.length > 0,
      degradationNotes
    };
  }

  compareRoots(a: Hash, b: Hash): "MATCH" | "DIVERGENCE" {
    return a === b ? "MATCH" : "DIVERGENCE";
  }

  exportLog(): EventLog {
    return {
      genesisHash: this.log.genesisHash,
      manifestRef: this.log.manifestRef,
      events: this.log.events.map(e => ({ ...e, canonicalPayload: new Uint8Array(e.canonicalPayload) }))
    };
  }
...
function testReplayParity(): boolean {
  ...
  for (const event of events) {
    const a = kernelA.appendEvent(event);
    const b = kernelB.appendEvent(event);
    if (a !== b) return false;
  }
