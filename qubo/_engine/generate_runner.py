...
if __name__=="__main__":
    main()
'''
leaf = sys.argv[1] if len(sys.argv)>1 else "leaf003"
src = ROOT / "data" / leaf / "claims.json"
outdir = ROOT / "qubo" / leaf
out = outdir / "run_qubo.py"
if not src.exists():
    raise SystemExit(f"MISSING_INPUT: {src}")
outdir.mkdir(parents=True, exist_ok=True)
out.write_text(TEMPLATE.replace("__LEAF_ID__", leaf))
out.chmod(0o755)
