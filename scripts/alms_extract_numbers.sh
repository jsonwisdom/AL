...
usage() {
  cat <<'EOF'
Usage:
  echo "Balance $3.7B [source:url:https://example.com/report.pdf|sha256:<64hex>]" | scripts/alms_extract_numbers.sh
...
def parse_anchor(body):
    ...
    bl = b.lower()
    if bl == 'inline':
        return {'type':'inline','location':None,'expected_raw_hash':None}
    if bl.startswith('sentence:'):
        return {'type':'sentence','location':b.split(':',1)[1],'expected_raw_hash':None}
    if bl.startswith('citation:'):
        return {'type':'citation','location':b.split(':',1)[1],'expected_raw_hash':None}
    if bl.startswith('url:'):
        payload = b[4:]
        expected = None
        if '|sha256:' in payload:
            loc, h = payload.rsplit('|',1)
            expected = h.lower()
        else:
            loc = payload
        return {'type':'url','location':loc,'expected_raw_hash':expected}
    return {'type':'unknown','location':b,'expected_raw_hash':None}
...
def anchor_for_number(num_end, next_num_start):
    ...
    for start,end,obj in anchors:
        ...
        if start - num_end <= 240: candidates.append((start,obj))
