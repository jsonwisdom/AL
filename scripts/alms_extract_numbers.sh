...
def parse_anchor(body):
    ...
    if bl.startswith('url:'):
        payload=b[4:]; parts=payload.split('|')
        obj={'type':'url','location':parts[0],'expected_raw_hash':None,'ipfs_cid':None,'ens_key':None}
        for part in parts[1:]:
            p=part.strip(); pl=p.lower()
            if pl.startswith('sha256:'): obj['expected_raw_hash']=pl
            elif pl.startswith('ipfs:'): obj['ipfs_cid']=p.split(':',1)[1]
            elif pl.startswith('ens:'): obj['ens_key']=p.split(':',1)[1]
        return obj
    return {'type':'unknown','location':b,'expected_raw_hash':None,'ipfs_cid':None,'ens_key':None}

anchors=[(m.start(),m.end(),parse_anchor(m.group('body'))) for m in anchor_pattern.finditer(text)]
text_for_numbers=anchor_pattern.sub('', text)
...
def canonical_decimal(value):
    return str(value.quantize(Decimal('1'))) if value == value.to_integral_value() else format(value.normalize(),'f')
...
matches=list(number_pattern.finditer(text_for_numbers)); numbers=[]
