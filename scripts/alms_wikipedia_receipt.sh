#!/usr/bin/env bash
set -euo pipefail
echo "🌐 ALMS Receipt #7 - Wikipedia Pageviews (Attention Signal)"
source .venv/bin/activate
source .alms_env
mkdir -p _truth/bigquery
OUT="_truth/bigquery/wikipedia_pageviews_ny_top10.csv"
python3 <<'PY' > "$OUT"
...
=== DATASET_RECEIPT ===
TIMESTAMP: $TS
DATASET: wikipedia_pageviews_top10
